...
4. Compose the change set through the engine generator commands so deltas are built deterministically, not by hand-writing `schema.json`:
   - New controlled value (`domain`, `source_kind`, `capture_type`): `propose-property --property <name> --value <value>`. `status` and note `type` are not edited this way.
   - New note type: `propose-note-type --name <slug> --description <text> --folder <path>` (see "Defining a new note type").
   - Templates, indexes, hubs: `propose-template`, `propose-index`, `propose-topic-hubs`.
5. Schema changes are never unattended safe approvals. Validate the proposal (dry-run review), obtain explicit approval, apply deterministically, rewrite the norms lock (`norms-lock --write`), and revalidate representative notes and Bases.
...
2. Run `propose-note-type` with the chosen slug, description, and preferred folder. The proposal writes the schema delta, the `note-types/<slug>.md` template, and creates the folder.
