...
4. Ask the user to decide durable purpose, retrieval priorities, ignored paths, folder depth, note types, properties, tags, links, and automation limits.
...
Keep all vault-specific policy and generated state inside the selected system folder. Treat `.pi-vault/config.yaml` only as the bootstrap locator.
