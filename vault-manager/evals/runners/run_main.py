"""Run one served main model over the gold note set and write a graded result.

Only one large model fits in VRAM, so this evaluates a single model per run:

    python -m evals.runners.run_main --model qwen3.6-27b-q6

Switch the served model, re-run with the matching --model key, and report.py
will aggregate every evals/results/main_*.json into one leaderboard.
...
def run(model_key: str, *, vault_root: Path, limit: int | None, embed_url: str | None,
    ...
    cfg = load_yaml(CONFIGS_DIR / "main_models.yaml")
    model_cfg = next((m for m in cfg["models"] if m["key"] == model_key), None)
    if model_cfg is None:
        raise SystemExit(f"unknown model key `{model_key}` (see configs/main_models.yaml)")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate one served main model.")
    parser.add_argument("--model", required=True, help="model key from configs/main_models.yaml")
    ...
    parser.add_argument("--tag", default="",
                        help="suffix the result file (main_<key>_<tag>.json) so a partial-stage "
    ...
    out_path = RESULTS_DIR / f"main_{args.model}{suffix}.json"
