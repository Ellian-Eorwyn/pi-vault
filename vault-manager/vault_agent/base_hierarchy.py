...
def generate_base_hierarchy_proposal(
    *,
    ...
    operations: list[dict[str, Any]] = [
        {
            "op": "write_file",
            "path": f"{output}/Vault Domains.md",
            "if_exists": "overwrite",
    ...
    for domain in plan.domains:
        operations.append(
            {
                "op": "write_file",
                "path": f"{output}/{_safe_filename(domain.label)}.md",
                "if_exists": "overwrite",
        ...
        for parent in domain.parent_groups:
            operations.append(
                {
                    "op": "write_file",
                    "path": f"{output}/{_safe_filename(domain.label)}/{_safe_filename(parent.label)}.md",
                    "if_exists": "overwrite",
...
def render_domain_dashboard(*, domain: DomainGroup, output_root: str) -> str:
    parent_links = "\n".join(
        f"- [[{output_root}/{_safe_filename(domain.label)}/{_safe_filename(parent.label)}|{parent.label}]] ({parent.count})"
        for parent in domain.parent_groups
    )
    if not parent_links:
        parent_links = "- No parent/project dashboards met the threshold."
    frontmatter = _dashboard_frontmatter(domain=domain.domain, parent="[[Vault Domains]]")
...
def _domain_summary_line(domain: DomainGroup, output_root: str) -> str:
    link = f"[[{output_root}/{_safe_filename(domain.label)}|{domain.label}]]"
