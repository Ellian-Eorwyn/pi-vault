...
def generate_property_proposal(
    *,
    ...
    # The canonical schema note renders per-value definitions from schema.json, so a
    # supplied description is stored there (the retired 0.021 doc no longer exists).
    definition_key = DEFINITION_SCHEMA_KEYS.get(property_name)
    if definition_key and (description or "").strip():
        definitions = deepcopy(schema.get(definition_key) or {})
